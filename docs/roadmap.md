...
当前进度：已完成纯文本摄取合同、确定性切片、TXT/Markdown/JSON/CSV 解析、PDF/DOCX/XLSX 可选解析器、有界内存开发队列、PostgreSQL 持久化任务、Redis Stream 消费者组、任务租约/自动重试/崩溃接管、失败任务查询与人工重试、死信流和数据保留清理、BGE-M3/BGE-Reranker 惰性适配、Milvus 3.x Dense+BM25+RRF 混合检索、低分拒答、MCP 摄取/查询边界、双重权限过滤与引用生成，以及 Recall@K/MRR/负例拒答/权限泄漏离线门禁和首批受控 golden set。待完成 OCR、缓存、检索策略路由、真实业务集扩充和生成 faithfulness 评测。
...
当前进度：NebulaGraph 参数化适配器、关系类型白名单、最大 4 跳、返回数量上限、租户/权限标签默认拒绝策略和演示建图脚本已完成。待启动真实集群验证返回值解析、CDC、审核、版本与回滚流程。

## M4：记忆与生产治理

1. Redis session TTL、摘要压缩、大结果对象存储引用。
2. 用户记忆确认 UI、business memory 审批流和全量审计。
3. token/时延预算、缓存、熔断、人工介入队列。API/工具/LLM 三层超时、重试、熔断、舱壁、内存限流和 Redis 原子分布式限流实现已完成；待真实 Redis 故障注入和告警面板验收。
